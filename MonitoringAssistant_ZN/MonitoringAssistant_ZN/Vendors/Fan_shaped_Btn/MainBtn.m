...
- (void)setSubBtns:(NSArray *)subBtns{
...
- (void)initSubBtn
{
    if (!_btnsAry) {
        ...
        for (int index = 0; index < self.subBtns.count; index ++) {
            ...
            btn.circleCenter = CGPointMake(_centerView.center.x, _centerView.center.y);
...
- (void)showSubBtnAnimation:(NSInteger )index
{
    ...
    if (btn) {
        [UIView animateWithDuration:(0.0f / self.subBtns.count) animations:^{
            btn.hidden = NO;
            btn.alpha = 1.0f;
...
- (void)layoutSubviews
{
    ...
    for (FanBtn *btn in _btnsAry) {
        ...
        btn.pressBtnColor = self.pressSubBtnColor;
        btn.iconImgName = self.subBtns[index];
    }
    [self initShapeLayer];
}
